...
struct ngx_pool_cleanup_s {
    ...
    void                 *data;     // 指向存储的数据地址
    ngx_pool_cleanup_t   *next;     // 下一个 ngx_pool_cleanup_t
};
...
struct ngx_pool_s {
    ...
    ngx_pool_large_t     *large;    // 当申请内存大于max时，单独分配一块大的内存块，存储在该链表
    ngx_pool_cleanup_t   *cleanup;  // 可自定义回调函数，清除内存块分配的内存，例如文件描述符
    ngx_log_t            *log;      // 日志
};
